...
print('BOPTEST case selected: {}'.format(testcase))

if os.path.isdir(os.path.join('models', testcase)):
    print('Initiate REST API for BOPTEST case: {}'.format(testcase))
    
    fmu_path = os.path.join('models', testcase, 'wrapped.fmu')  
    case = TestCase(fmu_path)
    
else:
    print('FMU path {} does not exists'.format(os.path.join('models', testcase)))
# ---------------------

...
model TwoZones
  ...
  inner Modelica.Blocks.Noise.GlobalSeed globalSeed annotation(
    Placement(visible = true, transformation(origin = {-92, 162}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Noise.BandLimitedWhiteNoise w_s(samplePeriod = 10, useAutomaticLocalSeed = true, useGlobalSeed = true)  annotation(
    Placement(visible = true, transformation(origin = {100, 24}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor senTZonNor annotation(
    Placement(visible = true, transformation(extent = {{60, 50}, {80, 70}}, rotation = 0)));
  Buildings.Utilities.IO.SignalExchange.Read TRooAirNor(KPIs = Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.AirZoneTemperature, description = "Zone air temperature of north zone", y(unit = "K"), zone = zonNorName) annotation(
    Placement(visible = true, transformation(extent = {{80, 120}, {60, 140}}, rotation = 0)));
  Modelica.Blocks.Math.Gain effNor(k = 1 / 0.99) annotation(
    Placement(visible = true, transformation(extent = {{0, 150}, {20, 170}}, rotation = 0)));
  Modelica.Blocks.Math.Abs absNor annotation(
    Placement(visible = true, transformation(extent = {{30, 150}, {50, 170}}, rotation = 0)));
  Buildings.Utilities.IO.SignalExchange.Read PHeaNor(KPIs = Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.GasPower, description = "Heater power of north zone", y(unit = "W")) annotation(
    Placement(visible = true, transformation(extent = {{80, 150}, {100, 170}}, rotation = 0)));
equation
  connect(resSou.port_b, capSou.port) annotation(
    Line(points = {{20, 0}, {40, 0}}, color = {191, 0, 0}));
  connect(capSou.port, senTZonSou.port) annotation(
    Line(points = {{40, 0}, {60, 0}}, color = {191, 0, 0}));
  connect(preTOut.port, resSou.port_a) annotation(
    Line(points = {{-20, 30}, {-10, 30}, {-10, 0}, {0, 0}}, color = {191, 0, 0}));
  connect(souTOut.y, preTOut.T) annotation(
    Line(points = {{-79, 30}, {-42, 30}}, color = {0, 0, 127}));
  connect(conSou.y, oveActSou.u) annotation(
    Line(points = {{-49, -30}, {-44, -30}}, color = {0, 0, 127}));
  connect(oveActSou.y, preHeaSou.Q_flow) annotation(
    Line(points = {{-21, -30}, {0, -30}}, color = {0, 0, 127}));
  connect(preHeaSou.port, capSou.port) annotation(
    Line(points = {{20, -30}, {40, -30}, {40, 0}}, color = {191, 0, 0}));
  connect(oveActSou.y, effSou.u) annotation(
    Line(points = {{-21, -30}, {-10, -30}, {-10, -80}, {-2, -80}}, color = {0, 0, 127}));
  connect(setSou.y, conSou.u_s) annotation(
    Line(points = {{-79, -30}, {-72, -30}}, color = {0, 0, 127}));
  connect(TRooAirSou.y, conSou.u_m) annotation(
    Line(points = {{59, -50}, {-60, -50}, {-60, -42}}, color = {0, 0, 127}));
  connect(effSou.y, absSou.u) annotation(
    Line(points = {{21, -80}, {28, -80}}, color = {0, 0, 127}));
  connect(absSou.y, PHeaSou.u) annotation(
    Line(points = {{51, -80}, {78, -80}}, color = {0, 0, 127}));
  connect(conCO2Sou.y, CO2RooAirSou.u) annotation(
    Line(points = {{141, -30}, {158, -30}}, color = {0, 0, 127}));
  connect(resNor.port_b, capNor.port) annotation(
    Line(points = {{20, 60}, {40, 60}}, color = {191, 0, 0}));
  connect(preTOut.port, resNor.port_a) annotation(
    Line(points = {{-20, 30}, {-10, 30}, {-10, 60}, {0, 60}}, color = {191, 0, 0}));
  connect(conNor.y, oveActNor.u) annotation(
    Line(points = {{-49, 100}, {-42, 100}}, color = {0, 0, 127}));
  connect(oveActNor.y, preHeaNor.Q_flow) annotation(
    Line(points = {{-19, 100}, {0, 100}}, color = {0, 0, 127}));
  connect(setNor.y, conNor.u_s) annotation(
    Line(points = {{-79, 100}, {-72, 100}}, color = {0, 0, 127}));
  connect(conCO2Nor.y, CO2RooAirNor.u) annotation(
    Line(points = {{141, 90}, {158, 90}}, color = {0, 0, 127}));
  connect(preHeaNor.port, capNor.port) annotation(
    Line(points = {{20, 100}, {40, 100}, {40, 60}}, color = {191, 0, 0}));
  connect(TRooAirSou.u, add.y) annotation(
    Line(points = {{82, -50}, {104, -50}, {104, -37}}, color = {0, 0, 127}));
  connect(senTZonSou.T, add.u2) annotation(
    Line(points = {{80, 0}, {98, 0}, {98, -14}}, color = {0, 0, 127}));
  connect(w_s.y, add.u1) annotation(
    Line(points = {{112, 24}, {138, 24}, {138, -4}, {108, -4}, {108, -14}, {110, -14}}, color = {0, 0, 127}));
  connect(capNor.port, senTZonNor.port) annotation(
    Line(points = {{40, 60}, {60, 60}}, color = {191, 0, 0}));
  connect(TRooAirNor.y, conNor.u_m) annotation(
    Line(points = {{59, 130}, {-60, 130}, {-60, 112}}, color = {0, 0, 127}));
  connect(senTZonNor.T, TRooAirNor.u) annotation(
    Line(points = {{80, 60}, {90, 60}, {90, 130}, {82, 130}}, color = {0, 0, 127}));
  connect(effNor.u, preHeaNor.Q_flow) annotation(
    Line(points = {{-2, 160}, {-10, 160}, {-10, 100}, {0, 100}}, color = {0, 0, 127}));
  connect(effNor.y, absNor.u) annotation(
    Line(points = {{21, 160}, {28, 160}}, color = {0, 0, 127}));
  connect(absNor.y, PHeaNor.u) annotation(
    Line(points = {{51, 160}, {78, 160}}, color = {0, 0, 127}));
  annotation (uses(Modelica(version="3.2.3"), Buildings(version="9.0.0")),
      experiment(
      StopTime=60,
      Interval=1,
      Tolerance=1e-06),

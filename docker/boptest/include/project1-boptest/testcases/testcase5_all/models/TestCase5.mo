...
model TestCase5
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor Rie(final R = RiSpe/area) annotation (
    Placement(visible = true, transformation(origin={6,0},    extent = {{-6, -6}, {6, 6}}, rotation = 0)));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor Rih(final R = RhSpe/area) "Thermal resistance between heater and interior" annotation (
    Placement(visible = true, transformation(origin = {74, -26}, extent = {{-6, -6}, {6, 6}}, rotation = -90)));
  ...
  Modelica.Blocks.Math.Gain Ai(k = 50)  annotation(
    Placement(visible = true, transformation(origin = {47, 55}, extent = {{-5, -5}, {5, 5}}, rotation = -90)));
  ...
  Modelica.Blocks.Logical.OnOffController onOffController(bandwidth = 1.0)  annotation(
  ...
  Buildings.Utilities.IO.SignalExchange.Read TRooHea(KPIs = Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.AirZoneTemperature, description = "Zone air temperature", y(unit = "K")) annotation(
    Placement(visible = true, transformation(origin = {120, -40}, extent = {{6, -6}, {-6, 6}}, rotation = 180)));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor temperatureSensor1 annotation(
    Placement(visible = true, transformation(origin = {78, 28}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
  Buildings.Utilities.IO.SignalExchange.Read TRooEnv(KPIs = Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.AirZoneTemperature, description = "Zone air temperature", y(unit = "K")) annotation(
    Placement(visible = true, transformation(origin = {114, 28}, extent = {{6, -6}, {-6, 6}}, rotation = 180)));
  Modelica.Blocks.Noise.BandLimitedWhiteNoise w_i(samplePeriod = 10, useAutomaticLocalSeed = true, useGlobalSeed = true) annotation(
    Placement(visible = true, transformation(origin = {147, 33}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
  Modelica.Blocks.Math.Add add(k1 = noisePower) annotation(
    Placement(visible = true, transformation(origin = {105, -23}, extent = {{-7, -7}, {7, 7}}, rotation = -90)));
...
equation
  ...
  connect(Ai.u, Solar.y) annotation(
    Line(points = {{47, 61}, {20, 61}, {20, 77}}, color = {0, 0, 127}));
  ...
  connect(Ai.y, solar_i.Q_flow) annotation(
    Line(points = {{47, 49.5}, {47, 46}, {42, 46}, {42, 26}}, color = {0, 0, 127}));
  ...
  connect(Rie.port_b, Ci.port) annotation(
    Line(points = {{12, 0}, {60, 0}}, color = {191, 0, 0}));
  ...
  connect(temperatureSensor.port, Ch.port) annotation(
    Line(points = {{58, -42}, {44, -42}, {44, -60}, {32, -60}, {32, -52}, {34, -52}}, color = {191, 0, 0}));
  connect(temperatureSensor.T, TRooHea.u) annotation(
    Line(points = {{70, -42}, {102, -42}, {102, -40}, {112, -40}}, color = {0, 0, 127}));
  connect(Ce.port, temperatureSensor1.port) annotation(
    Line(points = {{-18, 0}, {30, 0}, {30, 28}, {72, 28}}, color = {191, 0, 0}));
  connect(temperatureSensor1.T, TRooEnv.u) annotation(
    Line(points = {{84, 28}, {106, 28}}, color = {0, 0, 127}));
  connect(add.u1, w_i.y) annotation(
    Line(points = {{109, -15}, {160, -15}, {160, 33}, {155, 33}}, color = {0, 0, 127}));
  connect(add.u2, TRooEnv.y) annotation(
    Line(points = {{101, -15}, {108, -15}, {108, 10}, {132, 10}, {132, 28}, {120, 28}}, color = {0, 0, 127}));
  connect(add.y, TRooAir.u) annotation(
    Line(points = {{105, -31}, {106, -31}, {106, -64}, {72, -64}}, color = {0, 0, 127}));

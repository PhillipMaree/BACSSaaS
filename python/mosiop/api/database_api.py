...
class Database():
    ...
    def engine(self,db=None):
        db = 'postgres' if db is None else db
        return create_engine( self.cfg['type']+'+'+ self.cfg['connector']+ '://'+ self.cfg['user']+':'+ self.cfg['password']+'@'+self.cfg['host']+':'+self.cfg['port']+'/'+db)
    ...
    def connect(self,db=None):
        db = self.cfg['db'] if db is None or db not in self.list() else db
    ...
    def drop(self, *args):
        ...
        if len(args) == 1:
            db = args[0]
            if db == self.current():
                self.connect('postgres') # connect to default db
            if db in self.list():
    ...
    def has_table(self, schema, table, columns=[]):
        ...
        if len(columns)==0:
            return len(resp) > 0 and table in list(resp[0])
